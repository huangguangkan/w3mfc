...
CString CW3MFCRegistryMimeManager::GetMimeType(const CW3MFCRequest& request)
{
  ...
  szExt[0] = _T('\0');
  _tsplitpath_s(request.m_sLocalFile, nullptr, 0, nullptr, 0, nullptr, 0, szExt, sizeof(szExt)/sizeof(TCHAR));

  if (!_tcslen(szExt))
  	return CString(_T("application/octet-stream"));

  //Validate our parameters
  CString sExtension(szExt);
  ASSERT(sExtension.GetLength());
  ASSERT(sExtension[0] == _T('.'));

  //Prevent the string arrays from being manipulated by multiple threads at the one time
  ...
  if (!m_sMimeMap.Lookup(sTemp, sMimeType))
  {
    ...
    { 
      ...
      DWORD dwType = REG_SZ;
      if (RegQueryValueEx(hItem, _T("Content Type"), nullptr, &dwType, reinterpret_cast<LPBYTE>(sPath), &dwSize) == ERROR_SUCCESS)
      {
        sMimeType = sPath;
