...
History: PJN / 24-06-1999 1.  Implemented support for "HEAD" command
         ...
         PJN / 30-09-2001 1. Fixed a bug in CWSocket::Receive which can occur when a disconnection occurs
                          ...
                          case where it had embedded nulls e.g. a file upload. Thanks to Christian Hett for 
         ...
         PJN / 16-03-2014 1. Updated copyright details
                          2. Updated the code to compile cleanly on VC 2013
                          3. The sample app is now linked against the latest OpenSSL v1.0.1f dlls.
                          4. Reviewed all ASSERT calls for correctness
                          5. Reviewed all AFXASSUME calls for correctness
                          6. Updated #endif comments throughout the code base
                          7. By default W3MFC now builds using the non MFC build configuration for CWSocket and CWSocketException
                          8. Removed defunct code in CW3MFCClient constructor for old versions of Windows
                          9. Removed support for TransmitPackets and TransmitFile APIs from W3MFC for simplicity reasons.
                          10. Reworked CW3MFCClient::TransmitFile method.
         PJN / 26-11-2014 1. Updated the code to use the Author's SSLWrappers classes (http://www.naughter.com/sslwrappers.html) to provide the SSL
                          functionality for W3MFC instead of OpenSSL. Please note that the the W3MFC code when in SSL mode depends on the author's
                          CryptoWrappers classes (http://www.naughter.com/cryptowrappers.html) also. You will need to download both of these libraries
                          and copy their modules into the the W3MFC directory. Also note that the SSLWrappers and CryptoWrapper classes are only 
                          supported on VC 2013 or later and this means that the W3MFC SSL support is only supported when you compile with VC 2013 
                          or later. The solution files included in the W3MFC download are now for VC 2013.
         PJN / 17-07-2016 1. Updated copyright details.
         PJN / 22-11-2016 1. Removed unnecessary inclusion of atlcoll.h in stdafx.h from sample app.
                          2. Any errors which occur in the listener worker thread in CW3MFCServer::ListenSocketFunction are now correctly reported 
                          back to the main thread which starts it.
         PJN / 17-04-2017 1. Updated copyright details.
                          2. Updated the sample app and W3MFC to support a queue size different to the thread pool size. This new value is available
                          as CW3MFCSettings::m_nThreadPoolQueueSize
                          PJN / 28-04-2017 1. Updated the code to compile cleanly using /permissive-.
         PJN / 30-06-2017 1. Updated copyright details.
                          2. Updated the declaration of MFC synchronisation variables to work when ATL's atlsync.h is included in your project.
         PJN / 10-12-2017 1. Removed support for HTTP Keep-Alives. Removing this functionality means that the thread pool is W3MFC no longer
                          becomes blocked waiting for subsequent requests. Thanks to Olaf Rabus for reporting this issue.
                          2. Replaced NULL throughout the codebase with nullptr.
                          3. The stack size of the listen thread can now be customized via CW3MFCSettings::m_nListenThreadStackSize. Thanks to Olaf 
                          Rabus for suggesting this update.
                          4. Removed NTLM support from W3MFC as it requires Keep-Alives.
...
#include "CryptoWrappersStore.h" //If you get a compilation error about this missing header file, then you need to download my CryptoWrappers code from http://www.naughter.com/cryptowrappers.html
...
class CW3MFCDirectedThreadPoolQueue : public CDirectedThreadPoolQueue
{
public:
...
class CW3MFCServer : public IW3MFCError
{
public:
//Constructors / Destructors
  CW3MFCServer() : m_ListenStartEvent(TRUE),
                   m_pListenThread(nullptr),
                   m_dwListenerLastError(ERROR_SUCCESS)
  {
  }
...
//Methods
  virtual BOOL Start(CW3MFCSettings* pSettings)
  {
    CSingleLock sl(&m_csListenThread, TRUE); //synchronize access to the listening thread

    //Validate our parameters
    AFXASSUME(m_pListenThread == nullptr); //Trying to start an already started server
    AFXASSUME(pSettings != nullptr);
    AFXASSUME(pSettings->m_pMimeManager != nullptr); //must be assigned before you call this function
  #ifndef W3MFC_NO_ISAPI_SUPPORT
    AFXASSUME(pSettings->m_pISAPIManager != nullptr); //must be assigned before you call this function
    AFXASSUME(pSettings->m_pISAPI != nullptr); //must be assigned before you call this function
    pSettings->m_pISAPI->SetError(this);
  #endif //#ifndef W3MFC_NO_ISAPI_SUPPORT
  #ifndef W3MFC_NO_CGI_SUPPORT
    AFXASSUME(pSettings->m_pCGI != nullptr); //must be assigned before you call this function
    ...
    m_pListenThread = AfxBeginThread(ListenSocketFunction, this, THREAD_PRIORITY_NORMAL, pSettings->m_nListenThreadStackSize, CREATE_SUSPENDED);
    if (m_pListenThread == nullptr)
    {
      //Report the error
      CString sError;
      sError.Format(_T("CW3MFCServer::Start, Failed to create listener thread, please check settings, Error:%u"), ::GetLastError());
      OnError(sError);
      return FALSE;
    }
    m_pSettings = pSettings;
    ...
    SetLastError(m_dwListenerLastError);
    return (m_dwListenerLastError == ERROR_SUCCESS);
  }
  ...
  virtual BOOL Wait()
  {
    ...
    else
    {
      //Report the error
      CString sError;
      sError.Format(_T("CW3MFCServer::Wait, server is not running, so no need to wait"));
      OnError(sError);
      return FALSE;
    }
  }
...
//Accessors
  CW3MFCSettings* GetSettings() const 
  { 
    return m_pSettings; 
  }

  CThreadPoolServer<TCLIENT, TQUEUE>& GetThreadPool() 
  { 
    return m_ThreadPool; 
  }

  HANDLE GetImpersonationHandle() const 
  { 
...
//Methods
  ...
  virtual BOOL SetupSSL()
  {
    ...
    if (m_pSettings->m_bSSL)
    {
      ...
      if (!certStore.Open(CERT_STORE_PROV_SYSTEM, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, CERT_SYSTEM_STORE_CURRENT_USER | CERT_STORE_READONLY_FLAG, L"MY"))
      {
        m_dwListenerLastError = GetLastError();
        CString sError;
        sError.Format(_T("CW3MFCServer::SetupSSL, Failed to open certificate store, Error:%u"), m_dwListenerLastError);
        OnError(sError);
        return FALSE;
      }

      //Fill in an attribute structure for the certificate common name we are looking for
      CString sServerCertificateName(m_pSettings->m_sServerCertificateName);
      LPTSTR pszServerCertificateName = sServerCertificateName.GetBuffer();
      ...
      m_SSLCredentials.m_Certificate = certStore.FindCertificate(X509_ASN_ENCODING | PKCS_7_ASN_ENCODING,
                                                               #ifdef _UNICODE 
      ...
      if (m_SSLCredentials.m_Certificate.Handle() == nullptr)
      {
        m_dwListenerLastError = GetLastError();
        CString sError;
        sError.Format(_T("CW3MFCServer::SetupSSL, Failed to find certificate to use, Error:%u"), m_dwListenerLastError);
        OnError(sError);
        return FALSE;
      }

      memset(&m_SSLCredentials.m_sslCredentials, 0, sizeof(m_SSLCredentials.m_sslCredentials));
      m_SSLCredentials.m_sslCredentials.dwVersion = SCHANNEL_CRED_VERSION;
      m_SSLCredentials.m_sslCredentials.grbitEnabledProtocols = m_pSettings->m_dwSSLEnabledProtocols;
      m_SSLCredentials.m_sslCredentials.cCreds = 1;
      m_SSLCredentials.m_sslCredentials.paCred = &m_SSLCredentials.m_Certificate.m_pCert;
      SECURITY_STATUS status = m_SSLCredentials.Acquire(SECPKG_CRED_INBOUND, &m_SSLCredentials.m_sslCredentials);
      if (status != SEC_E_OK)
      {
        m_dwListenerLastError = status;
        CString sError;
        sError.Format(_T("CW3MFCServer::SetupSSL, Failed to setup SSL server credentials, Error:%08X"), m_dwListenerLastError);
        OnError(sError);
        return FALSE;
      }
    }
  ...
  virtual void ListenSocketFunction()
  {
    ...
    if (!m_ThreadPool.Start(m_pSettings->m_nThreadPoolSize, m_pSettings->m_nThreadPoolQueueSize, TRUE))
    {
      //Report the error
      m_dwListenerLastError = GetLastError();
      CString sError;
      sError.Format(_T("CW3MFCServer::ListenSocketFunction, Failed to create thread pool, Error:%u"), m_dwListenerLastError);
      OnError(sError);
    ...
    for (int i=0; i<m_pSettings->m_nThreadPoolSize; i++)
    {
      ...
      pClient->m_pError = this;
      pClient->m_pSettings = m_pSettings;
    #ifdef W3MFC_SSL_SUPPORT
      pClient->SetCachedCredentials(&m_SSLCredentials);
      pClient->SetReadTimeout(m_pSettings->m_dwSSLReadTimeout);
      pClient->SetWriteTimeout(m_pSettings->m_dwSSLWriteTimeout);
    #endif
      HANDLE hWorkerThread = pClient->GetThreadHandle();
      AFXASSUME(hWorkerThread != nullptr);
      if (ResumeThread(hWorkerThread) == static_cast<DWORD>(-1))
      {
        //Report the error
        m_dwListenerLastError = GetLastError();
        CString sError;
        sError.Format(_T("CW3MFCServer::ListenSocketFunction, Failed to resume worker thread at index %d in thread pool, Error:%u"), i, m_dwListenerLastError);
        OnError(sError);
    ...
    if (!m_ThreadPool.SetMaxThreadClientLifetime(m_pSettings->m_bEnableThreadLifetime, m_pSettings->m_dwThreadLifetime))
    {
      ...
      m_dwListenerLastError = GetLastError();
      CString sError;
      sError.Format(_T("CW3MFCServer::ListenSocketFunction, Failed in call to m_ThreadPool.SetMaxThreadClientLifetime, Error:%u"), m_dwListenerLastError);
      OnError(sError);
    ...
    {
      //Report the error
      m_dwListenerLastError = pEx->m_nError;
      CString sError;
      sError.Format(_T("CW3MFCServer::ListenSocketFunction, Failed to bind server socket, Error:%u"), m_dwListenerLastError);
      OnError(sError);
    ...
    {
      //Report the error
      m_dwListenerLastError = e.m_nError;
      CString sError;
      sError.Format(_T("CW3MFCServer::ListenSocketFunction, Failed to bind server socket, Error:%u"), m_dwListenerLastError);
      OnError(sError);
    ...
    catch(CWSocketException* pEx)
    {
      ...
      m_dwListenerLastError = pEx->m_nError;
      CString sError;
      sError.Format(_T("CW3MFCServer::ListenSocketFunction, Failed to listen on server socket, Error:%u"), m_dwListenerLastError);
      OnError(sError);
    ...
    if (bUseAccount)
    {
      ASSERT(m_pSettings->m_hImpersonation.m_h == nullptr);
      bLoggedOn = LogonUser(m_pSettings->m_sUserName, nullptr, m_pSettings->m_sPassword, LOGON32_LOGON_NETWORK, LOGON32_PROVIDER_DEFAULT, &m_pSettings->m_hImpersonation.m_h);
      if (bLoggedOn)
        bImpersonated = ImpersonateLoggedOnUser(m_pSettings->m_hImpersonation);
      else
      {
        //Report the error
        m_dwListenerLastError = ::GetLastError();
        CString sError;
        sError.Format(_T("CW3MFCServer::ListenSocketFunction, Failed to logon using user name: %s, Error:%u"), m_pSettings->m_sUserName.GetString(), m_dwListenerLastError);
        OnError(sError);
      }
      ...
      CW3MFCRequest::SecureEmptyString(m_pSettings->m_sPassword);
    } 
    ...
    {
      //Report the error
      OnError(_T("CW3MFCServer::ListenSocketFunction, Failed to impersonate using supplied user credentials"));
    ...
    if (nError == SOCKET_ERROR)
    {
      ...
      m_dwListenerLastError = GetLastError();
      CString sError;
      sError.Format(_T("CW3MFCServer::ListenSocketFunction, Failed in call to WSAEventSelect, Error:%u"), m_dwListenerLastError);
      OnError(sError);
    ...
    //We're now ready for accepting client connections, inform the main thread that everthing is ok
    m_dwListenerLastError = ERROR_SUCCESS;
    m_ListenStartEvent.SetEvent();
      ...
      else if (nSignaledHandle == 1) //It was a socket event
      {
        ...
        try
        {
          ...
          if (!pQueue->PostRequest(poolRequest))
          {
            ...
            //Report the error
            CString sError;
            sError.Format(_T("CW3MFCServer::ListenSocketFunction, An error occurred posted the request to the thread pool, Error:%u"), ::GetLastError());
            OnError(sError);
          }
        }
      #ifdef CWSOCKET_MFC_EXTENSIONS
        catch(CWSocketException* pEx)
        {
          ...
          //Report the error
          CString sError;
          sError.Format(_T("CW3MFCServer::ListenSocketFunction, An error occurred accepting a client connection, Error:%d"), pEx->m_nError);
          OnError(sError);
        ...
        catch(CWSocketException& e)
        {
          ...
          //Report the error
          CString sError;
          sError.Format(_T("CW3MFCServer::ListenSocketFunction, An error occurred accepting a client connection, Error:%d"), e.m_nError);
          OnError(sError);
        }
      #endif //#ifdef CWSOCKET_MFC_EXTENSIONS
      }
...
//Member variables
  ...
  DWORD                               m_dwListenerLastError;
  CW3MFCSettings*                     m_pSettings;
#ifdef W3MFC_SSL_SUPPORT
  SSLWrappers::CCachedCredentials     m_SSLCredentials;
#endif
  CThreadPoolServer<TCLIENT, TQUEUE>  m_ThreadPool;
};

#endif //#ifndef __W3MFC_H__
